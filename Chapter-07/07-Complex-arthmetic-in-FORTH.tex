...
Complex numbers can be used to represent points in the $x$-$y$ plane with $y$ plotted vertically and $x$ horizontally\sepfootnote{07_04}. ( Sometimes this graphical representation is called an \textbf{Argand plot} or \textbf{Argand diagram}\index{Argand!-plot, -diagram, -plane}. The $x$-$y$ plane is then called the \textbf{Argand plane}.)
...
\begin{figure}[h]
    ...
    \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
        ...
        \begin{tikzpicture}[ scale=1.5, ]
            ...
            % Axes
            \draw[-{Stealth[length=2mm]}] (-2.0, 0.0) -- (2.5,0.0) node[below left] {Re(z)};
            \draw[-{Stealth[length=2mm]}] ( 0.0,-0.5) -- (0.0,2.5) node[below left] {Im(z)};

            % Origin
            \node[below left] at (0,0) {0};

            % Define complex number z
            \coordinate (cor) at (1.5,1.5);

            % Projections
            \draw[dashed] (cor) -- (cor |- 0,0) node[below] {$x$};
            \draw[dashed] (cor) -- (cor -| 0,0) node[left]  {$y$};

            % Vector from origin to z
            \draw[very thick, -{Stealth[length=1.2mm]}] (0,0) -- (1.4,1.4) node[midway, above, sloped] {$r$};
            \fill (cor) circle (1.5pt) node[above right] {$z$};

            % Angle theta
            \draw[->] (0.7,0) arc[start angle=0, end angle=36.87, radius=0.7] node[midway, left] {$\theta$};
    ...
    \fbox{
        \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
            ...
            \begin{tikzpicture}[ scale=1, ]
                % Axes
                \draw[-{Stealth[length=2mm]}] (-3, 0.0) -- (3,0) node[below left] {Re(z)};
                \draw[-{Stealth[length=2mm]}] ( 0,-2.5) -- (0,3) node[below left] {Im(z)};

                % Complex number z in the 3rd quadrant
                \coordinate (zcor) at (-2,-1.5);
                \draw[thick, -{Stealth[length=2mm]}] (0,0) -- (zcor) node[below left] {$z$};

                % Projections for z
                \draw[] (zcor) -- (zcor |- 0,0) node[above] {$x$};
                \draw[] (zcor) -- (zcor -| 0,0) node[right] {$y$};

                \coordinate (w) at (-0.8, 1.3);
                \draw[thick, -{Stealth[length=2mm]}] (0,0) -- (w) node[above left] {$w$};
                \node at (1.75, 1.55) {$w = \sqrt{z}$};

                % Projections for w
                \draw[] (w) -- (w |- 0,0) node[below] {$a$};
                \draw[] (w) -- (w -| 0,0) node[right] {$b$};
...
\section{Complex exponentials and trigonometric functions} \index{complex functions!exponential}\index{complex functions!trigonometric}
...
\section{Logarithms} \index{complex functions!logarithm}
