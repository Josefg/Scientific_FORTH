...
\chapter{The 80x87 Family} \index{"!"2@80486DX cpu/fpu} \index{"!"4@80x87 co-processor family}

\TallC{When} we speak of the 80x87 mathematical co-processor family, we include the original Intel chips, the Cyrix D387 and IIT C287 and C387 chips, and the AMD 80287 and 80387 clones, as well as the on-chip floating point unit found on the Intel 80486 chips.

We now describe some features of the 80x87 floating point co-processors (FPUs) that affect scientific programming on IBM-PC compatible machines\sepfootnote{04_01}. The 8087 chip complements the Intel 8088/8086 CPU family, the 80287 works with the 80286 series, the 80387 with the 80386, and the 80486 includes an on-chip FPU. The 8087 chip is connected pin-for-pin to the 8088/8086. (The details are given in \textit{The 8087 Primer\sepfootnote{04_02}}) The interface and instruction set automatically take care of bus arbitration (that is, which chip has access to the memory) and interrupts, in order to be sure that the CPU and 80x87 do not perform conflicting operations.
...
We shall see in Ch. 4 \S1.1 how to use the FORTH assembler, and in Ch. 4 \S1.2 how to use DEBUG\sepfootnote{04_03}, to extend a FORTH system for 80x87 operations if they are not already included as a floating point lexicon.
...
Advanced programming methods --such a recursive algorithms-- require an fstack of unlimited depth. The designers of the 8087 anticipated the need for fstack extension and included instructions for this purpose. Unfortunately, the instructions were not well thought out\sepfootnote{04_04} so a moderately complex software fstack extension manager is needed to augment them. We design such a manager in Ch. 4 \S7.

\section{Internal 80x87 and manipulation} \index{"!"4@87stack}
\TallC{The} 80x87 is organized around a stack of 8 80-bit registers (the 87stack). The 8-deep stack can be subdivided into smaller stacks for special purposes, but this is only useful when coding in assembler for speed\sepfootnote{04_05}.
...
\subsection{The FORTH assembler}\index{assembler}

\TallC{Every} FORTH worthy of the name includes an assembler, usually set up as an alternate \textbf{vocabulary}\sepfootnote{04_06} called \bc{ASSEMBLER}. The assembler allows direct definition of a new word in terms of machine codes, which are referred to using standard mnemonics. A typical FORTH assembly language definition (we now specialize to HS/FORTH) for @ would have the form\sepfootnote{04_07}

\begin{lstlisting}
    CODE  @  BX [BX] MOV. END-CODE
\end{lstlisting}

A \bc{CODE}\index{assembler!CODE~...~END-CODE@CODE~...~END-CODE} definitionis a machine-coded subroutine somewhere in memory. To use it, the compiler has to know where it is and insert appropriate unconditional JUMP (JMP) and RETURN (RET) instructions in the machine code representation of the calling program.
...
\leftbar[1\linewidth]
\Note: the definition includes \regc{FWAIT}\index{assembler!FWAIT.@FWAIT.} (the same as \regc{WAlT}), an instruction that makes the 8086 CPU wait for the FPU to complete its work before attempting to access the memory. If \regc{WAlT} were omitted and the CPU accessed the memory, it could store incomplete results\footnotemark.
\endleftbar
\footnotetext{The design of the 80286, 80386, and 80486 eliminates this problem, consequently \regc{FWAIT} is not required when assembling 80287/80387/80487 machine code. See, \eg, John H. Crawford and Patrick P. Gelsinger, \textit{Programming the 80386} (Sybex. San Francisco, 1987). HS/FORTH allows the user to choose which class of machine to assemble for, when loading the 80x87 assembler extension. The 80287+ option simply defines \bc{FWAIT.} as a null word.}

\subsection{Using MS-DOS DEBUG}

\TallC{The}FORTH assembler, with its 80x87 extension, lets us develop machine-coded words while retaining Intel mnemonics for documentation, at the price of loading and compiling the entire \bc{ASSEMBLER} lexicon. But if we know the actual machine code bytes we can bypass the assembler by entering the (hexadecimal) codes directly into a \bc{CODE} definition. Most FORTH systems, in addition to an assembler, provide a way to insert machine codes --hex numbers-- directly into the code field of a word. HS/FORTH, \eg, uses the words \bc{<\%} and \bc{\%>}\index{assembler!$<$\%~...~\%$>$} to enclose the hex codes being inserted.

The problem is, how do we find out what these hex codes are?

The simplest way is to use DEBUG\sepfootnote{04_09} to generate (HEX) code sequences. Rather than try to explain, we shall illustrate by recording and annotating the DEBUG session for the word \bc{FROT}:
...
\section{Memory usage (storage and retrieval)} \index{assembler!FILD.@FILD.|(}\index{assembler!FISTP.@FISTP.|(}

\TallC{The} 80x87 instruction set includes codes for loading ST(0) from memory and storing ST(0) to memory. The former involeves a "push" and the latter may or may not involve a "pop", from
the 87stack.
...
Similarly, \bc{I32@} and \bc{I32!} can be defined by replacing \bc{WORD-PTR}\index{assembler!WORD-PTR@WORD-PTR} with \bc{DWORD-PTR}\index{assembler!DWORD-PTR@DWORD-PTR}. To define \bc{I64@} and \bc{I64@} --assuming these are needed-- replace \bc{DWORD-PTR} with \bc{QWORD-PTR}. The 32-, 64- and 80-bit floating point analogues \bc{R32@}, \bc{R32!}, \bc{R64@}, \bc{R64!}, \bc{R80@} and \bc{R80!} are defined byreplacing \bc{FILD} by \bc{FLD}\index{assembler!FLD.@FLD.} and \bc{FISTP} by \bc{FSTP}\index{assembler!FSTP.@FSTP.}, and using \bc{DWORD-PTR}, \bc{QWORD-PTR}\index{assembler!QWORD-PTR@QWORD-PTR} or \bc{TBYTE-PTR}\sepfootnote{04_10}\index{assembler!TBYTE-PTR@TBYTE-PTR} as appropriate.
...
Faster machine code versions of \bc{S->F} and \bc{F->S} are\sepfootnote{04_11}
...
\endleftbar
\footnotetext{See, \eg, John H. Crawford and Patrick P. Gelsinger, \textit{Progumming the 80386} (Sybex, San Francisco, 1987).}
\index{assembler!FILD.@FILD.|)}
\index{assembler!FISTP.@FISTP.|)}

\section{Arithmetic words}\index{"!"1@80486DX cpu/fpu}

\TallC{FPU} (80x87) arithmetic is generally performed with the maximum precision allowed by the (80-bit) size of the registers\sepfootnote{04_13}.
...
\TallC{For} convenience the designers of the 8087 chip have arranged fast loading of certain constants into ST(0) of the fstack (TOS). The words that place these constants on the fstack, and the corresponding assembler mnemonics and (hex) codes are shown in Table \ref{tbl:04_01} below.
...
\section{Test words}\index{assembler!FCOMPP.@FCOMPP.|(}
...
\index{assembler!FCOM.@FCOM.}
\index{assembler!FCOMP.@FCOMP.}
\index{assembler!FTST.@FTST.}
...
The results of these comparisons are encoded as bits C3 (14) and C0 (8) of the 16-bit 80x87 STATUS register. In order to get these bits by bit-masking techniques, the STATUS register must be moved to the parameter stack\sepfootnote{04_14}. This is done with the the 80x87 instruction \regc{FSTSW}\index{assembler!FSTP.@FSTSW.}\index{assembler!FSTP.@FSTP.} \textit{via} the assembler sequence
...
Now we have to consider how to bit-mask the status integer left on the stack by \bc{FSTSW}. We use the logical \bc{AND} with 4000h og 0100h (\underline{Exercise}: Why?) to pick out C3 and C0. Now from \textbf{8087P}\sepfootnote{04_15} we have the truth table \ref{tbl:04_03} below:

% Table 04-03 Truth table of test ...
\begin{table}
...
\bc{F0<}, as well as \bc{F>}, \bc{F=}, \bc{F<}. We have\sepfootnote{04_16}
...
\begin{lstlisting}
    ...
    DECIMAL
\end{lstlisting}
\index{assembler!FCOMPP.@FCOMPP.|)}
\section{Mathematical functions}

\TallC{We} now proceed to develop a suite of special functions for the 80x87 FPU. These will include the usual trigonometric functions, logarithms, and exponentials. We retain initial \bc{F}s in the names to remind us the FPU is being used. The functions are given in Table \ref{tbl:04_04} below:
...
\end{lstlisting}
\footnotetext{See below for a discussion of \bc{FSCALE}.}
...
A further problem created by the 8087/80287 instruction set is that the argument $z$ must lie in the range $0 < z < \pi/4$. Thus we must shift the argument to this range, using a special instruction \bc{FPREM} ("exact" partial remainder\sepfootnote{04_18}) that can be used to extract multiples of $\pi$:
...
\leftbar[1\linewidth]
\Note: The 80387 improves on the 8087/80287 by eliminating the need to adjust the argument in software. Further, the tangent produced by the 80387 is normalized (that is, $x = 1.0$ in Eq. \ref{eq:04_01} above). Finally, the 80387 has instructions \regc{FSIN}, \regc{FCOS}, and \regc{FSINCOS} built in, so all the software emulation is unnecessary.
\endleftbar
...
\section{Extending the intrinsic 80x87 stack} \index{"!"4@87stack}
\label{sec:Extending}
\TallC{As} promised in the beginning of the chapter, we now design a fstack manager in software that allows more than 8 cells. First we examine the structure of the 80x87 stack (\textbf{87stack}).

From \textbf{8087P} we see that the 8 registers in the 87stack are organized as a circular stack. A 3-bit pointer, \regc{ST}, records which physical register is actually TOS. The instruction set allows \regc{ST} to be incremented (\regc{FINCSTP})\index{assembler!FINCSTP@FINCSTP} or decremented (\regc{FDECSTP})\index{assembler!FDECSTP.@FDECSTP.} modulo 8. The 87stack is shown below in Fig. \ref{fig:04_01} on page 81.
...
\begin{itemize}
    \item When the 87stack gets full, put \regc{ST(7)} on the memory extensions and \textit{vice-versa}.
...
\begin{itemize}
    ...
    \item An \regc{FPUSH} or an \regc{FPOP} takes about 50 $\mu$sec on the average, when coded by hand, @ 4.77 MHz, or about 240 clock cycles.
    \item The irreducible minimum on an 8086/80286 --since \regc{TREAL} storage from, or retrieval to the 87stack' rs demanded-- cannot possibly be less than 170 clock cycles: 1 fetch and 1 store\sepfootnote{04_19}. (The main place to save some time would be in moves to or from memory; the depth of the fstack and the pointer must be kept as variables, but \bc{FS-SIZE} and \bc{FLGTH} do not change and could be compiled as literals, thereby saving 30 cycles in \bc{FPUSH} and 10 in \bc{FPOP}.)
...
Then we load the 4$\times$4 matrix into banks 1 and 2, the vector into bank 0, and multiply:
...
\TallC{The} on-chip 4$\times$4 matrix multiply was intended to accelerate 3-dimensional graphics (rotation and translation can be expressed as a single 4-dimensional transformation). However, most scientific programmers spend little time on 3-D graphics. The matrix instructions are more interesting for their potential to accelerate general matrix operations\sepfootnote{04_21}. For example, suppose we need to transform a vector by multiplying with an arbitrary matrix. Normally we would write

\begin{align}
    y_{i} = \sum_{j=1}^{n} A_{ij}x_{j}
\end{align}

But consider, \eg, an 8$\times$8 matrix operating on an 8-dimensional column vector: we \textbf{partition} the matrix and vector into 4-dimensional sub-matrices $a_{11}$ , \textit{etc}. and sub-vectors $x_{1}$ , \textit{etc}.:
...
as for \textit{matrix$\cdot$vector}. If one must load the 4$\times$4 matrix each time, the speedup factor is less: about 3.5-fold because of the 16 additional fetches.

The conventionally programmed 8$\times$8 matrix-vector multiply should also be some 3-5 times faster than the scalar operations, \textit{i.e.} there is no obvious speed gain --except being able to employ the built-in vector instruction \bc{F4x4}-- from partitioning the 8$\times$8 system into 4$\times$4 sub-units. However, Strassen\sepfootnote{04_22} has pointed out that if one can evaluate matrix products recursively, partitioning can substantially speed the most time-consuming matrix operations, multiplication, and inversion. For example, it appears as though the product of two partitioned matrices,
...
\begin{align}
    \widehat{M}_{2n} = 7\widehat{M}_{n} + 18an^{2}
\end{align}

or

\begin{align}
    \widehat{M}_{n} \approx mn^{lg\ 7} - 6an^{2} ,
\end{align}
...
which leads to the recursion for $I_{a}$, the time to invert an n$\times$n matrix:
\begin{align}
I_{2n} \cong&\ 2I_{n} + 5M_{n}
\end{align} 
...
\TallC{Suppose} we merely wish to solve a linear system \textit{without} inverting the matrix: can we gain some speed that way? From 17 we
