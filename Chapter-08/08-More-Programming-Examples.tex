...
(x-a_{n})p_{n-2} \eqiv \sum_{k=0}^{n-1} \mu_{k}p_{k}(x)
\end{equation}
...
One way to accomplish the minimization is via calculus, using a method known as \textbf{steepest descents}. The idea is to differentiate
...
\textit{Numerical Recipes}\sepfootnote{08_26}, as an example of just how indecipherable
...
ecall (Ch. 8, \S1.1) how we approximated the area under a curve
...
\TallC{In the program} \bc{)INTEGRAL} in Ch.8 \S1\S\S5.3 the word \bc{INTERPOLATE} performs Richardson extrapolation for the trapezoid»
...
Equation \ref{eq:08_52b} is only an approximation because the R that ap-
...
Equation 53 is exactly what appears in \bc{INTERPOLATE}.
