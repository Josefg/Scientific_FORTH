...
The definite integral $\int_{a}^{b}f(x) dx$ is the area between the graph of the function and the x-axis as shown below in \ref{fig:08_01}:

\input{Chapter-08/fig_08_01}
...
\subsubsection{A simple Monte-Carlo program}\label{chap:08_01_03_02}
...
\begin{figure}[H]
    ...
    \begin{tabular}{>{\ttfamily}l ll}
    \multicolumn{3}{l}{784 48 TRACE GCD} \\
    \quad & \underline{rstack} & \underline{stack}  \\
    : GCD                                     &           &  784 48        \\
    \quad DUP                                 &           &  784 48  48    \\
    \quad 0$=$4                               &           &  784 48  0     \\
    \quad 0BRANCH$<$ 8 $>$0                   &           &  784 48        \\
    \quad UNDER                               &           &  48  784 48    \\
    \quad MOD                                 &           &  48  16        \\
    \quad : GCD                               &           &  48  16        \\
    \quad \quad  DUP                          & 4B76      &  48  16  16    \\
    \quad \quad  0$=$                         & 4B76      &  48  16  0     \\
    \quad \quad  0BRANCH$<$ 8 $>$0            & 4B76      &  48  16        \\
    \quad \quad  UNDER                        & 4B76      &  16  48  16    \\
    \quad \quad \quad MOD                     & 4B76      &  16  0         \\
    \quad \quad \quad : GCD                   & 4B76      &  16  0         \\
    \quad \quad \quad \quad DUP               & 4B76 4B76 &  16  0   0     \\
    \quad \quad \quad \quad 0$=$              & 4B76 4B76 &  16  0   65535 \\
    \quad \quad \quad \quad 0BRANCH$<$8$>$-1  & 4B76 4B76 &  16  0         \\
    \quad \quad \quad \quad DROP              & 4B76 4B76 &  16            \\
    \quad EXIT                                & 4B76      &  16            \\
    EXIT                                      &           &  16            \\
    \end{tabular}
\end{figure}
...
Step 5: N=N-2=2 \quad I=I+($I'_2$+$I_3$)+($I'_2$+$I_3$-$I_2$)/3
\input{Chapter-08/08_step_5}

The flow diagram of the algorithm now looks like Fig. \ref{fig:08_04} below:

\input{Chapter-08/fig_08_04}

and the resulting FORTH program is\sepfootnote{08_15}:

\input{Chapter-08/08_code_task_integral}
...
\input{Chapter-08/Table_08_5_4_Integral_in_use}
...
The calculus of residues (Cauchy's theorem) gives\index{Cauchy's theorem!calculus of residues}
...
\input{Chapter-08/Table_08_6_Argand_1}
\input{Chapter-08/Table_08_6_Argand_2}
...
There are several ways to speed up the computation of a function. They are all based on compact representations of the funtion --either in tabular form or as coefficients of functions that are faster to evaluate. For example, we might represent sin(x) by a simple polynomial\sepfootnote{08_18}\index{Abramowitz, M. and Stegun, I.A.}
...
Although the FFT algorithm can be programmed recursively, it almost never is. To see why, imagine how the coefficients would be re-shuffled by Eq. \ref{eq:08_30}: we work out the case for 16 coefficients, exhibiting them in Table \ref{table:08_01} below, writing only the indices:

\begin{table}[h]
    \caption{Bit-reversal for re-ordering discrete data}
    \fbox{
...
\TallC{One} such improvement is a word to reverse the bits in a given integer. Note how clumsily this was done in the FORTRAN program.
...
13  CONTINUE                  \ end outer loop
    MMAX=ISTEP
    GO TO 2
  ENDIF         \ end Danielson-Lancszos section
  RETURN
    \end{verbatim}
\end{multicols}

Since practically every microprocessor permits right-shifting a register one bit at a time and feeding the overflow into another register from the right, \bc{B.R} can be programmed easily in machine code for speed. Our fast bit-reversal procedure \bc{B.R} may be represented pictorially as in Fig. 8-5 below.

\input{Chapter-08/fig_08_05}
...
Now, how do we go about evaluating the sub-polynomials to get the answer? First, let us write the polynomials (for our case N = 16) corresponding to taking the (bit-reversed) addresses off the stack in succession, as in Fig. \ref{fig:08_06} below.

\input{Chapter-08/fig_08_06}
...
\TallC{Now} we test the program. Table \ref{table:08_02} on page~\pageref{table:08_02} contains the weekly stock prices of IBM stock, for the year 1983 (the 52 values have been made complex numbers by adding $0i$, and the table padded out to 64 entries (the nearest power of 2) with complex zeros)\sepfootnote{08_22}. The first two entries (2,64) are the type and length of the file. (The file reads from left to right.)

We FFl‘ Table \ref{table:08_02} using the phrase \bc{IBM\{ 64 DIRECT \}FFT}. The power spectrum of the resulting FFT (Table 8-3) is shown in Fig. \ref{fig:08_07} on page~\pageref{fig:08_07} below.

\input{Chapter-08/08_code_Complex_FFT}

\TallC{How} do we know the FFT program actually worked? The simplest method is to inverse-transform the transform, and compare with the input file. The FFT and inverse FFT are given, respectively, in Tables \ref{table:08_03} and \ref{table:08_04} on page~\pageref{table:08_02} below. Within roundoff error, Table \ref{table:08_04} agrees with Table \ref{table:08_02} on page~\pageref{table:08_02}.

    \input{Chapter-08/Table_08_02}
    \input{Chapter-08/fig_08_07}
    \input{Chapter-08/Table_08_03}
    \input{Chapter-08/Table_08_04}
...
\input{Chapter-08/fig_08_09_star}
...
\input{Chapter-08/08_code_Gram_polynomials}
...
\input{Chapter-08/08_code_FORTRAN_simplex_method}
\input{Chapter-08/08_code_minimization_by_simplex_method}