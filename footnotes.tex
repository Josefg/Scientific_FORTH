...
%
%8. A OMnputer stores a number with finite precision — say 6-7 decimal pUces with 32-bit floating-
%point numbers. This is enough for many purposes, especiaUy in science and engineering, where
%the data are rarely measured to better than 1% reUtive precision. Suppose, however, that two
%numbers, about IO' 2 in magnitude, are mukipUed. Their product is of order IO^ and is known to
%six significant figures. Now add k to a third number of order unity. The result wUl be that third
%number ± 1 0 ^ . Later, a fourth number — ako of order unity — is subtracted from this sum. The
%resuh wiU be a number of order 10^, but now known onfy to two «gn«fir*m figures. Matrix arith-
%metic is fuU of muUipUcations and additions. The fesson is dear — to M i m m i ^ the (inevfcabk)
%k m of precision Msoriatrd with roundoff, we must Uy to keep the * " g p ^ ^ H of products and
%sunu as dose as potsiMe
