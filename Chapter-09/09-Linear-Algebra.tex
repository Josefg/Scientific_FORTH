...
Now we can redefine \bc{\}\}R1-R2*X} using vectored execution, \textit{via}
HS/FORTH's \bc{CASE: ... ;CASE} construct, or the equivalent high-level version given here:
...
We now combine the words \bc{\}SWAP}, \bc{\}\}PIVOT}, \bc{\}\}ROW*X}.
\bc{\}R1-R2*X} and \bc{\}V1-V2*X} to implement the triangularization portion of pivotal elimination.
...
\TallC{We} introduce this subject with a brief discourse on liners
...
(Eq. \erratumReplace{typo}{4}{\ref{eq:09_03}}-- 1's on the main diagonal, 0's everywhere else). We have
